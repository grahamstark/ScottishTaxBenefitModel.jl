...
const NON_CALCULATED = fill( WAGES, PENSION_CONTRIBUTIONS_EMPLOYER )
const BENEFITS = fill(CHILD_BENEFIT,SCOTTISH_CARERS_SUPPLEMENT ) # careful!
const LEGACY_MTBS =fill(WORKING_TAX_CREDIT,HOUSING_BENEFIT)

const MEANS_TESTED_BENS = fill(WORKING_TAX_CREDIT,UNIVERSAL_CREDIT)
const NON_MEANS_TESTED_BENS = setdiff( BENEFITS, MEANS_TESTED_BENS )
const INCOME_TAXES = fill(INCOME_TAX,NATIONAL_INSURANCE)

const CALCULATED = fill(INCOME_TAX,SCOTTISH_CARERS_SUPPLEMENT)
const SICKNESS_ILLNESS = fill(SEVERE_DISABILITY_ALLOWANCE,DLA_MOBILITY)
const DEDUCTIONS = fill(HEALTH_INSURANCE,PENSION_CONTRIBUTIONS_EMPLOYER)
