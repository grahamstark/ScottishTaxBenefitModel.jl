...
export CARERS_ALLOWANCE_SUPPLEMENT
export BASIC_INCOME
export DISCRETIONARY_HOUSING_PAYMENT
export CHILD_DISABILITY_PAYMENT_CARE
export CHILD_DISABILITY_PAYMENT_MOBILITY
export PENSION_AGE_DISABILITY
export ADP_DAILY_LIVING
export ADP_MOBILITY
export CARERS_SUPPORT_PAYMENT
export OTHER_SCOTTISH_BENEFITS
export OTHER_TAX
...
const MEANS_TESTED_BENS = union(fill(WORKING_TAX_CREDIT,UNIVERSAL_CREDIT), [SCOTTISH_CHILD_PAYMENT,COUNCIL_TAX_BENEFIT,DISCRETIONARY_HOUSING_PAYMENT] )
const NON_MEANS_TESTED_BENS = IncomesSet(setdiff( BENEFITS, MEANS_TESTED_BENS ))
const INCOME_TAXES = make_income_taxes()

const CALCULATED = fill(INCOME_TAX,typemax( Incomes ))
...
const SCOTTISH_BENEFITS = union( 
    ...
    [SCOTTISH_CHILD_PAYMENT,DISCRETIONARY_HOUSING_PAYMENT] ) # FIXME plus ...
...
const ALL_INCOMES_EXCEPT_HOUSING_BENEFITS = IncomesSet(setdiff(ALL_INCOMES,[COUNCIL_TAX_BENEFIT,HOUSING_BENEFIT])) # FIXME?? ,DISCRETIONARY_HOUSING_PAYMENT

const DIRECT_TAXES_AND_DEDUCTIONS = union(INCOME_TAXES,DEDUCTIONS,[LOCAL_TAXES] )
...
const EXEMPT_INCOME = IncomesSet([
    ...
    GUARDIANS_ALLOWANCE,
    HOUSING_BENEFIT,
    INCAPACITY_BENEFIT, # 1st 24 weeks
    INCOME_SUPPORT,
    INDUSTRIAL_INJURY_BENEFIT,
    MATERNITY_ALLOWANCE,
    MATERNITY_GRANT,
