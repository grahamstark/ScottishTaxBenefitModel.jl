...
# ╔═╡ 47b2abf4-f1ab-11ee-2937-ed41a3e3131d
begin
	using CairoMakie,DataFrames,CSV,StatsBase, CategoricalArrays, Format
end

# ╔═╡ c7a1722f-612e-4a0c-876a-273b108e1bcb
begin

function fm(x) 
	Format.format(x;precision=0,commas=true)
end
...
# ╔═╡ 97c42099-d77e-43a8-9f8a-c3608a6b8f3b
CIVIL_COSTS_GRP

# ╔═╡ 883b828f-2960-4009-b914-27815bbd9ed2
levels( CIVIL_AWARDS.case_status )

# ╔═╡ 741be93c-92c7-4b76-ab1e-658ba7b75106
begin
	function oneof( x, things... )
		return if x ∈ things
			true
		else        
         	false
...
# ╔═╡ 731d6aa0-761a-4ab5-b1f1-eccbc4bed872
begin
	civtype = groupby( CIVIL_AWARDS, [:hsm,:sex,:age ])
	nr = length( civtype )
	d = DataFrame( 
		typ=fill("", nr), 
		age=fill("",nr), 
		sex=fill("",nr), 
		granted = zeros(Int,nr), 
		refused=zeros(Int,nr))
	p = 0
	for( k, v ) in pairs( civtype )
		global p
		p += 1
		# println(k)
		sex = ismissing(k.sex) ? "Missing" : k.sex
		age = ismissing(k.age) ? "Missing" : k.age
		d.typ[p] = k.hsm
		d.sex[p] = sex
		d.age[p] = age
		d.granted[p] = count( oneof.(v.case_status, 	"Granted","Approved","Continued","Granted With Contribution"))
		d.refused[p] = length( v.case_status ) - d.granted[p]
	end
	d.total = d.refused .+ d.granted
	d.refusedpct = 100 .* (d.refused ./ d.total)
	sort!(d, [:typ,:sex,:age])
	total = sum( d.granted ) + sum( d.refused )
	d,total,fm(size(CIVIL_AWARDS)[1])
end

# ╔═╡ 59941266-e761-4b9e-b6b1-8ab4017f024d
begin
	xcivtype = groupby( CIVIL_AWARDS, [:hsm ])
	xnr = length( xcivtype )
	xd = DataFrame( 
		typ=fill("", xnr), 
		granted = zeros(Int,xnr), 
		refused=zeros(Int,xnr))
	xp = 0
	for( xk, xv ) in pairs( xcivtype )
		global xp
		xp += 1
		# println(k)
		xd.typ[xp] = xk.hsm
		xd.granted[xp] = count( oneof.(xv.case_status, 	"Granted","Approved","Continued","Granted With Contribution"))
		xd.refused[xp] = length( xv.case_status ) - xd.granted[xp]
	end
	xd.total = xd.refused .+ xd.granted
	xd.refusedpct = 100 .* (xd.refused ./ xd.total)
	sort!(xd, [:typ])
	xtotal = sum( xd.granted ) + sum( xd.refused )
	xd,xtotal,sum(xd.granted)
end

# ╔═╡ 0071c637-02c3-495c-bbec-6611ef22e530
begin
...
# ╔═╡ df84f8b5-d957-4cf9-b6b7-21ec351df738
summarystats( cv[cv .> 0])
...
manifest_format = "2.0"
project_hash = "f5788ef069a31ccd9c1a954be2242dbe3852063f"
